...
let mostRecentPunishment = reactive({ data: null });
let banned = false;
...
if (response.ok) {
  ...
  user.data = json[route.params.user];
} else {
...
if (infoResponse.ok) {
  ...
  mostRecentPunishment.data = json.punishments.pop();
  userInfo.data = json.user;
  userRank.data = json.grants.pop();
  banned = mostRecentPunishment.data.removedAt == null && (mostRecentPunishment.data.expiresAt != null ? Date.now() < mostRecentPunishment.data.expiresAt : true);
} else {
    ...
    <main class="main">
      ...
      <div class="row">
        <div class="col-lg-3">
          <aside id="sidebar">
            <div class="box">
              <div class="banned-box" v-if="banned">
                <span class="text">
                  <strong class="title">Banned</strong>
                  {{ formatTimeAgo(userInfo.data.lastSeenAt) }}
                </span>
              </div>
              <div class="online-box" v-else-if="userInfo.data.online">
                <span class="text">
              ...
              <div class="offline-box" v-else>
                <span class="text">
                  ...
                  Last Seen {{ formatTimeAgo(userInfo.data.lastSeenAt) }}
                </span>
              </div>
